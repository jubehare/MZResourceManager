namespace MZResourceManager.Models;

public static class MzEventCode
{
    public const int ShowText = 101;
    public const int ShowTextBody = 401;
    public const int ShowChoices = 102;
    public const int ChoiceBranch = 402;
    public const int InputNumber = 103;
    public const int ShowScrollingText = 105;
    public const int ScrollingTextBody = 405;
    public const int ConditionalBranch = 111;
    public const int CallCommonEvent = 117;
    public const int ControlSwitches = 121;
    public const int ControlVariables = 122;
    public const int ChangeItems = 126;
    public const int TransferPlayer = 201;

    public const int ShowPicture = 231;
    public const int ChangeBattleback = 333;
    public const int PlayBgm = 241;
    public const int PlayBgs = 245;
    public const int PlayMe = 249;
    public const int PlaySe = 250;
    public const int PlayMovie = 261;
    public const int BattleProcessing = 301;
    public const int ShopProcessing = 302;
    public const int ChangeActorName = 320;
    public const int ChangeActorNickname = 324;
    public const int Script = 355;
    public const int PluginCommandMv = 356;
    public const int PluginCommand = 357;

    public static string Label(int code) => code switch
    {
        ShowText => "Show Text (face)",
        ShowTextBody => "Dialogue",
        ShowChoices => "Show Choices",
        ChoiceBranch => "Choice",
        InputNumber => "Input Number",
        ShowScrollingText => "Scroll Text",
        ScrollingTextBody => "Scroll Text body",
        ConditionalBranch => "Conditional Branch",
        CallCommonEvent => "Call Common Event",
        ControlVariables => "Control Variables",
        TransferPlayer => "Transfer Player",
        ShowPicture => "Show Picture",
        PlayBgm => "Play BGM",
        PlayBgs => "Play BGS",
        PlayMe => "Play ME",
        PlaySe => "Play SE",
        PlayMovie => "Play Movie",
        BattleProcessing => "Battle Processing",
        ShopProcessing => "Shop Processing",
        ChangeActorName => "Change Actor Name",
        ChangeActorNickname => "Change Actor Nickname",
        Script => "Script",
        PluginCommandMv => "Plugin Command (MV)",
        PluginCommand => "Plugin Command",
        _ => $"Code {code}",
    };
}
